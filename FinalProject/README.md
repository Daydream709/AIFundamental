...
**自研模型改进入口**：
- `models/kan_iTransformer.py`（4 模块集成的高性能模型，~120M）
...
│   ├── kan_iTransformer.py         # ⭐ 自研高性能模型 (4 模块集成)
...
4 模块集成，每个可独立关闭（对应 Line 4a 的 4 档 ablation）：
...
3. **概率输出** (GaussianNLL) — 输出 (mean, logvar)，支持不确定性
4. **模型仲裁** (`layers/meta_arbitrator.py`) — 基于 5 维统计特征动态选模型
...
### 5.5 消融 4a：KAN-iTransformer 4 模块

```bash
python scripts/train_line4a_kan.py
```

- 4 消融设置 (A0 完整 / A1 w/o CFD / A2 w/o 概率输出 / A3 w/o RevIN) × 3 数据集 × 1 pred_len = **12 runs**
- A0 / A1 / A3 必跑；A2 跑过即可
- M5 大约 1.5 小时
...
**入口**：`models/kan_iTransformer.py`（~300 行），4 模块分布在 `layers/` 下。
...
| 🟡 中 | 模型仲裁 (模块 4) 加更多 meta-features（目前 5 维：谱熵/趋势/周期/方差/自相关）| `layers/meta_arbitrator.py` | Line 4a (A4 关仲裁) |
| 🟡 中 | 模块 3 概率输出：尝试 learned heteroscedastic 或 mixture of Gaussians | `exp/exp_train.py` | Line 2 (看 NLL/CRPS) |
