...
KAN-iTransformer 基于 iTransformer 倒置架构（自注意力在变量维而非时间维），集成 4 个核心模块：

**1. RevIN 可逆实例归一化**：在编码前对输入按变量计算均值/标准差并归一化，解码后做反归一化，消除训练-测试之间的分布偏移。**消融实验里贡献最大的模块**（去掉后 Electricity MSE 上升 7.4%）。

**2. 3 路并行 KAN 专家**：将 iTransformer 单路 FFN 替换为 3 个并行的 KAN 层（命名上分别对应 trend / seasonal / residual 三个分量）。3 个 KAN 都接收编码器归一化后的特征，各自用 B-spline 基函数独立拟合；在训练中通过梯度下降**隐式分化**——分别捕获不同周期 / 频率成分的响应模式（短周期 vs 长周期、平稳段 vs 突变段），但**不显式做频域分解**。

**3. 自适应门控融合**：将 3 个 KAN 专家的输出在特征维拼接 → 轻量门控网络（`Linear → GELU → Linear → Softmax`）得到 3 个权重 → 按权重加权组合作为最终输出。门控让模型根据输入特性动态决定依赖哪个 KAN 专家（例如平稳段偏重 trend_kan，突变段偏重 residual_kan）。
...
**图 5-5** 给出 2 个自研模型 + SparseTSF（轻量基线）在 3 个数据集上与**该数据集最佳基线**的相对差距（柱状图，负值=超越基线）。最佳基线因数据集而异：ETTm2 → **DLinear**（MSE 0.1016），Weather → **TimesNet**（MSE 0.1602），Electricity → **PatchTST**（MSE 0.1618）：

![图 5-5：自研模型 vs 最佳基线（含 SparseTSF 对照）](figures/fig5_innovative_vs_baseline.png)

_图 5-5：2 个自研模型 + SparseTSF（轻量基线）在 ETTm2/Weather/Electricity 上对**各数据集最佳基线**（ETTm2=DLinear / Weather=TimesNet / Electricity=PatchTST）的相对差距。**橙色 KAN-iTransformer 在 Electricity 上以 -12.3% 显著超越基线**；**红色 SparseTSF 在 Electricity 上以 +264% 完全失效**（柱子被截断显示，作为轻量基线的边界性能对照）。_

**表 5-8：自研模型 + SparseTSF 与各数据集最佳基线的相对差距**（负值=超越基线，正值=落后基线）

| 模型             | ETTm2 (vs DLinear)  |  Weather (vs TimesNet)  | Electricity (vs PatchTST) | 适用场景               |
| ---------------- | :----: | :-------: | :---------: | ---------------------- |
...
- **SparseTSF（轻量基线）**：仅在低维强周期数据上有效，**不适合本研究的部分场景（高维多变量）**——但因其结构简单、参数量极小（0.015M），是后续多模态融合实验的理想载体（详见 5.4 节）
...
![图 5-10：7 模型 × 4 数据集的 FLOPs（fvcore 测量, 对数坐标）](figures/fig10_params_flops.png)

_图 5-10：7 模型在 4 个数据集上的浮点运算。TimesNet 在 Electricity 上暴涨（143.99G），KAN-iTransformer 在所有数据集上保持最高（0.1-2.8G），LiteSparseNet 在 Electricity 上膨胀到 0.004G，DLinear/SparseTSF 始终最小（< 0.01G）。_
...
![图 5-11：7 模型 × 3 数据集的推理时间与显存（line1/line2 CSV 测量, F=96, 对数坐标）](figures/fig11_infer_memory.png)

_图 5-11：7 模型在 3 个数据集上的推理时间（左, ms）和显存占用（右, MB）。推理最快是 DLinear（0.09ms），SparseTSF 在 Electricity 上推理极慢（47ms / 42MB 是 321 客户独立 Linear 的代价）；显存最低 DLinear（19-41MB），KAN-iTransformer 稳定在 1GB 左右，TimesNet/PatchTST 在 Electricity 上分别 3.8GB / 25.7GB。_
...
**关键洞察 6：LiteSparseNet 推理时间极短，仅次于 DLinear**

从 7 模型推理时间排序（ETTm2, F=96）来看：DLinear 0.09ms < **LiteSparseNet 0.19ms** < KAN-iTransformer 1.50ms < SparseTSF 1.17ms < PatchTST 2.45ms < Mamba 4.95ms < TimesNet 5.41ms。LiteSparseNet 的推理时间仅比 DLinear 多 0.1ms（2.1×），但**比 PatchTST 快 13×、比 Mamba/TimesNet 快 26-28×**。这意味着在边缘部署场景里，LiteSparseNet 可以作为 DLinear 推理速度的"等价替代品"——保留了 0.04M 参数量级（vs PatchTST 10.1M），同时获得相近的实时性。
...
**模型**：选用 **SparseTSF**（轻量化基线）作为多模态载体，验证文本融合对其时序预测的增益（使用轻量化模型主要是为了加快实验速度）。
...
### 5.5 主线 4：自研模型的模块消融实验

**定位说明**：在完成本主线的过程中，我发现现实约束是：要在短短数周内，对已有 SOTA 模型做出**架构级**的改进几乎是不可能的——这些模型的原作者已经做了大量调参、试错和工程优化。因此，本研究在后面进行的过程中**不追求在精度或效率上超越原始工作**，而是把自研模型当作**可拆解的容器**，通过系统地打开 / 关闭各模块，**量化每个组件对最终性能的贡献**，并借此探索不同技术（KAN、RevIN、频域分解的替代设计、稀疏连接结构等）在不同场景、不同数据集（低维 ETTm2 / 中维 Weather / 高维 Electricity / 多模态 Environment）下的**相对优劣**。换言之，我们更关心的是"**为什么 X 有效 / Y 在哪些场景下失效**"，而不是"**让 X 达到新 SOTA**"。
...
1. **RevIN 是最关键的组件**：去掉 RevIN 在 Electricity 上 MSE 上升 7.4%（0.1427 → 0.1533），在 ETTm2 上上升 3.9%。RevIN 的归一化/反归一化操作对高维数据尤为重要。**为什么**：RevIN 把"量级"这个无关变量消掉后，模型只需专注学"模式"本身；Electricity 321 客户量级差异极大（工业 vs 居民差几个数量级），收益最大；ETTm2 仅 7 个变量且量级相近，收益降至 3.9%。

2. **3 路 KAN 专家的贡献中等**：将 3 路 KAN 退化为单 KAN（`use_cfd=False` 触发 `SimpleKANEncoderLayer`，仅保留 1 路 KAN FFN）后，ETTm2 上 MSE 上升 3.2%，Electricity 上上升 2.0%。但在 Environment 上反而下降 4.0%，说明对日频短序列数据，1 个 KAN 已足够，3 路专家的隐式分化收益有限。**为什么**：3 路专家的隐含假设是"输入有多个显著频率分量可被分工捕获"；Environment 是日频数据、频率结构稀疏，拆 3 路反而让每个专家**样本不足而欠拟合**；ETTm2/Electricity 高频数据频率结构丰富，3 路专家才有用武之地。

3. **概率输出影响不一致**：去掉概率输出在 ETTm2 上上升 1.6%，在 Environment 上上升 4.0%，但在 Electricity 上略有改善。概率输出对低维数据有帮助，对高维数据作用不大。**为什么**：GaussianNLL 的 log-variance 项是**自适应正则器**——对预测把握不大时自动放大方差、降低对均值的损失权重；Electricity 60M 参数已"学得太满"，再引入 log-variance 轻微过拟合；Environment 参数量小、噪声相对大，不确定性估计帮助模型不去拟合噪声。
...
4. **多模态融合有效**：`search`（实时舆情）单独使用为 SparseTSF 带来 1.6-3.9% MSE 改善，`search+report` 简单拼接几乎无额外增益。

---

## 参考文献

[1] Zeng, A., Chen, M., Zhang, L., & Xu, Q. (2023). Are Transformers Effective for Time Series Forecasting?. _Proceedings of the AAAI Conference on Artificial Intelligence, 37_(9), 11121–11128.

[2] Zhou, H., Zhang, S., Peng, J., Zhang, S., Li, J., Xiong, H., & Zhang, W. (2021). Informer: Beyond Efficient Transformer for Long Sequence Time-Series Forecasting. _Proceedings of the AAAI Conference on Artificial Intelligence, 35_(12), 11106–11115.

[3] Wu, H., Xu, J., Wang, J., & Long, M. (2021). Autoformer: Decomposition transformers with auto-correlation for long-term series forecasting. _In Advances in Neural Information Processing Systems_.

[4] Nie, Y., Nguyen, N. H., Sinthong, P., & Kalagnanam, J. (2023). A Time Series is Worth 64 Words: Long-term Forecasting with Transformers. _In International Conference on Learning Representations_.

[5] Liu, Y., Hu, T., Zhang, H., Wu, H., Wang, S., Ma, L., & Long, M. (2024). iTransformer: Inverted Transformers Are Effective for Time Series Forecasting. _In The Twelfth International Conference on Learning Representations_.

[6] Wu, H., Hu, T., Liu, Y., Zhou, H., Wang, J., & Long, M. (2023). TimesNet: Temporal 2D-Variation Modeling for General Time Series Analysis. _In The Eleventh International Conference on Learning Representations_.

[7] Gu, A., & Dao, T. (2023). Mamba: Linear-Time Sequence Modeling with Selective State Spaces. _arXiv preprint arXiv:2312.00752_.

[8] Liu, Z., Wang, Y., Vaidya, S., Ruehle, F., Halverson, J., Soljačić, M., Hou, T. Y., & Tegmark, M. (2025). KAN: Kolmogorov-Arnold Networks. _In The Thirteenth International Conference on Learning Representations_.
...
# 主线 4a: KAN-iTransformer 模块消融（4 配置 × 3 数据集 = 12 次）
python scripts/train_line4a_kan.py --epochs 30 --pred_len 96

# 主线 4b: LiteSparseNet 残差瓶颈消融（3 配置 × 3 数据集 = 9 次）
python scripts/train_line4b_lite.py --epochs 30 --pred_len 96
```
