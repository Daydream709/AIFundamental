...
2. **精度与效率的权衡问题**：在边缘部署场景下，模型参数量必须控制在极低水平（< 0.05M）；而在高精度优先的场景下，又需要集成 KAN（用可学习 B-spline 替代固定激活函数）、频域分解、概率输出等前沿手段。本研究分别从两个方向探索：**自研高性能模型 KAN-iTransformer**（KAN + iTransformer + 频域分解 + 概率输出 + RevIN 五大模块）冲刺精度上限；**自研轻量模型 Lite-SparseNet**（稀疏跨周期下采样 + 分组轻量 MLP + 线性残差）以 < 0.05M 参数逼近中量级模型的精度。
...
| **主线 3: 多模态实验**   | 2 模型 × 1 数据集 × 5 text_mode     | 10 次    |
...
_图 5-5：3 个创新模型在 ETTm2/Weather/Electricity 上对最佳基线的相对差距。**橙色 KAN-iTransformer 在 Electricity 上以 -12.3% 显著超越基线**；**红色 SparseTSF 在 Electricity 上以 +264% 完全失效**（柱子被截断显示）。_
...
> **数据来源**：`results/line3_latest.csv`（20 runs = 2 架构 × 5 模态 × 2 pred_len）

#### 5.4.1 多模态实验设置

**模型**：选用 **PatchTST**（Transformer 代表）和 **Mamba**（SSM 代表）作为多模态载体。
...
| both_gating    | 时序 + 报告 + 搜索（门控） | 门控融合 vs 简单拼接  |

#### 5.4.2 实验结果

**表 5-9：PatchTST 和 Mamba 在 Environment × 5 模态 × 2 pred_len 上的 MSE**（`results/line3_latest.csv`）

| 模态               | PatchTST F=96 | PatchTST F=192 | Mamba F=96 | Mamba F=192 |
| ------------------ | :-----------: | :------------: | :--------: | :---------: |
| baseline（纯时序） |    0.4321     |     0.5063     |   0.2559   |   0.2160    |
| report             |    0.4321     |     0.5063     |   0.2559   |   0.2160    |
| search             |    0.4321     |     0.5063     |   0.2559   |   0.2160    |
| both_concat        |    0.4321     |     0.5063     |   0.2559   |   0.2160    |
| both_gating        |    0.4321     |     0.5063     |   0.2559   |   0.2160    |

#### 5.4.3 数据洞察分析

**核心发现：多模态机制在最新实验中未生效**

PatchTST 和 Mamba 的 5 种模态实验**每行 4 个 MSE 完全相同**（PatchTST F=96 全部 0.4321、F=192 全部 0.5063；Mamba F=96 全部 0.2559、F=192 全部 0.2160）。这说明 text_embed 在前向传播中被静默丢弃，模型实际只用了时序数值。

**与 SparseTSF 中间版本对比**：

| 模型                | 文本模态数  |                       是否生效                       | 来源                                 |
| ------------------- | :---------: | :--------------------------------------------------: | ------------------------------------ |
| PatchTST（最新）    |   5 模态    |                ❌ 5 模态 MSE 完全相同                | `results/line3_latest.csv`           |
| Mamba（最新）       |   5 模态    |                ❌ 5 模态 MSE 完全相同                | `results/line3_latest.csv`           |
| SparseTSF（中间版） | 4 text_mode | ✅ 4 种结果不同（0.5953 / 0.5920 / 0.5860 / 0.5862） | `results/line3_sparsetsf_latest.csv` |

**根因分析**：

1. **数据加载未将多模态信号送进训练循环**。`dataset_base.py` 把多模态字段放在 `batch[4]`（text），但 PatchTST/Mamba 的训练循环（`exp/exp_train.py`）只解包 `batch[0:4]`，把多模态字段**直接丢弃**。
2. **SparseTSF 中间版之所以"有效"**，是因为它重写了数据流——把 text_embed 显式塞进 forward 签名。但 PatchTST/Mamba 没有做相应改造，多模态就**从未被模型看到**。
3. **多模态信号即便被送入**，融合方式也是 `concat`（在投影后简单拼接），缺少显式的跨模态对齐（如 InfoNCE 对比损失、cross-attention）。

**修复方向**（已识别，未实施）：

- 在 `exp/exp_train.py` 中解包完整 batch，把 text_embed 送进模型 forward
- 在 `models/PatchTST.py` 等的 forward 签名增加 `text_embed=None` 可选参数
- 实现 GatingFusion / InfoNCE 等显式多模态融合模块

#### 5.4.4 资源消耗

虽然多模态未生效，但已记录的资源消耗如下（仅 baseline，因为 5 模态参数量相同）：

| 模型                   | 参数量 | 推理时间 (ms) | FLOPs (G) |
| ---------------------- | :----: | :-----------: | :-------: |
| PatchTST (Environment) | 6.904M |     1.55      |   29.4    |
| Mamba (Environment)    | 0.237M |     4.84      |   1.52    |

#### 5.4.5 多模态结论

1. **多模态机制未生效**：PatchTST 和 Mamba 的 5 种模态实验 MSE 完全相同（`results/line3_latest.csv`）
2. **根因明确**：训练循环丢弃了 `batch[4]`（text）
3. **可修复**：解包完整 batch + 在模型 forward 接收多模态字段 + 实现 GatingFusion
4. **SparseTSF 中间版提供正面证据**：当数据流正确时，文本融合可带来 1-4% MSE 改善
5. **架构 vs 数据流的优先级**：先修数据流，再谈多模态架构创新
...
主线 3 实验（详见 5.4 节）显示 PatchTST 和 Mamba 的 5 种模态 MSE 完全相同，多模态信号未被模型使用。根因已定位：

1. **数据流断裂（主因）**：`exp/exp_train.py` 训练循环只解包 `batch[0:4]`（x_enc, x_y, x_mark_enc, x_mark_y），丢弃 `batch[4]`（text_embed）。即便 `dataset_base.py` 正确生成了多模态字段，模型 forward 永远拿不到。
2. **架构未改造**：PatchTST / Mamba 的 forward 签名不接受 `text_embed` 可选参数。即便数据流修好，仍需在模型侧加融合接口。
3. **缺跨模态对齐**：即便修好数据流，简单的 `concat` 不够。文献 (Time-MMD) 显示有效融合需要 GatingFusion、InfoNCE 对比损失、cross-attention 等显式模块。
4. **SparseTSF 中间版的正面对照**：当数据流重写后（4 text_mode），4 种结果不同（0.5953 / 0.5920 / 0.5860 / 0.5862），证明**修复数据流即可让多模态有效**——不需要改架构就能拿到 1-4% 改善。

**修复路径**（已规划，未实施）：

- 第一步：在 `exp/exp_train.py` 中解包完整 batch，把 `text_embed` 传给 model
- 第二步：在 `models/PatchTST.py` 和 `models/Mamba.py` 的 forward 增加可选 `text_embed=None` 参数
- 第三步：实现 GatingFusion（已存在于 `layers/gating_fusion.py`）并在模型内调用
- 第四步：在 Environment 上重跑 Line 3，验证 5 种模态产生不同结果
...
2. **多模态机制已定位根因**：PatchTST/Mamba 的 5 种模态 MSE 完全相同（数据流断裂），SparseTSF 中间版数据流修复后能拿到 1-4% 增益
...
5. **多模态融合机制未生效**（数据流断裂，根因已定位），需修复 `exp/exp_train.py` 数据流。
